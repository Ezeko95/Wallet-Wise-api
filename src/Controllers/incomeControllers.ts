...
export const createIncome = async (infoIncome: IIncome) => {
    ...
    const balanceToChange = await BalanceModel.findOne({where: { id: infoIncome.accountId}})

    const total = balanceToChange?.total

    if(total !== undefined){
        const newTotal = total + infoIncome.amount
        await BalanceModel.update({ total: newTotal }, { where: { id: infoIncome.accountId } });
    }

    const finalBalance = await BalanceModel.findOne({
        where: { id: infoIncome.accountId},
        include:[ IncomeModel]
    })
...
export const deleteIncome = async (id: number) =>{
    ...
    const balanceIncome = income?.accountId
